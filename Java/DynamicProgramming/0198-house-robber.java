...
class Solution {
    public int rob(int[] nums) {
        ...
        for (int i=1; i<=nums.length-1; i++) {
            int maxSumForIndexMinusTwo = i-2 < 0 ? 0 : cache[i-2];
            int maxSumForIndexMinusOne = cache[i-1];
            cache[i] = Integer.max((nums[i] + maxSumForIndexMinusTwo), maxSumForIndexMinusOne);
        }
