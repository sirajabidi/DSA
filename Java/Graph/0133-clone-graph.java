...
class Solution {
    public Node cloneGraph(Node rootNode) {
        if (rootNode == null) {
            return null;
        }
        ...
        Queue<Node> readQueue = new LinkedList<>();
        readQueue.add(rootNode);

        Queue<Node> writeQueue = new LinkedList<>();
        Node rootNodeClone = new Node(rootNode.val);
        writeQueue.add(rootNodeClone);

        nodeCache.put(rootNode, rootNodeClone);

        while (!readQueue.isEmpty()) {
            Node currentFromReadQueue = readQueue.poll();
            Node currentFromWriteQueue = writeQueue.poll();

            for(Node neighbor : currentFromReadQueue.neighbors) {

                if (nodeCache.containsKey(neighbor)) {
                    currentFromWriteQueue.neighbors.add(nodeCache.get(neighbor));
                } else {
                    Node neighborClone = new Node(neighbor.val);
                    nodeCache.put(neighbor, neighborClone);
                    currentFromWriteQueue.neighbors.add(neighborClone);

                    readQueue.add(neighbor);
                    writeQueue.add(neighborClone);
                }
            }
        }
